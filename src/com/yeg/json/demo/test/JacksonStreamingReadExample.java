...
public class JacksonStreamingReadExample {
	...
	public static void main(String[] args) throws JsonParseException,
		...
		// File("employee.txt"));
		byte[] jsonData = Files.readAllBytes(Paths.get(Constant.JSON_TEST_DIR,
				"employee.txt"));
	...
	private static void parseJSON(JsonParser jsonParser, Employee emp,
		...
		while (jsonParser.nextToken() != JsonToken.END_OBJECT) {
			...
			} else if ("street".equals(name)) {// 地址类中街道
				jsonParser.nextToken();
				emp.getAddress().setStreet(jsonParser.getText());
			} else if ("city".equals(name)) {// 地址类中城市
				jsonParser.nextToken();
				emp.getAddress().setCity(jsonParser.getText());
			} else if ("zipcode".equals(name)) {// 地址类中邮编
				jsonParser.nextToken();
